...
public class SiteViewModel extends ViewModel {
    ...
    public void homeContent() {
        execute(result, () -> {
            Site site = SiteConfig.get().getHome();
            if (site.getType() == 3) {
                Spider spider = SiteConfig.get().getSpider(site);
                String homeContent = spider.homeContent(true);
                SpiderDebug.log(homeContent);
                SiteConfig.get().setRecent(site);
    ...
    public void categoryContent(String key, String tid, String page, boolean filter, HashMap<String, String> extend) {
        execute(result, () -> {
            Site site = SiteConfig.get().getSite(key);
            if (site.getType() == 3) {
                Spider spider = SiteConfig.get().getSpider(site);
    ...
    public void playerContent(String key, String flag, String id) {
        execute(player, () -> {
            ...
            if (site.getType() == 3) {
                Spider spider = SiteConfig.get().getSpider(site);
                String playerContent = spider.playerContent(flag, id, SiteConfig.get().getFlags());

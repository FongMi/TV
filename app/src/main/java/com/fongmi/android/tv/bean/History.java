...
public class History {
    ...
    public String getSiteName() {
        return SiteConfig.get().getSite(getSiteKey()).getName();
    }
    ...
    public static History find(String key) {
        return AppDatabase.get().getHistoryDao().find(SiteConfig.getCid(), key);
    }
    ...
    public History delete() {
        AppDatabase.get().getHistoryDao().delete(SiteConfig.getCid(), getKey());
        AppDatabase.get().getTrackDao().delete(getKey());
        return this;
    }

    public List<History> find() {
        return AppDatabase.get().getHistoryDao().findByName(SiteConfig.getCid(), getVodName());
    }
    ...
    private static void startSync(List<History> targets) {
        for (History target : targets) {
            ...
            if (items.isEmpty()) {
                target.update(SiteConfig.getCid(), items);
                continue;
            }
            for (History item : items) {

...
public class SiteAdapter extends RecyclerView.Adapter<SiteAdapter.ViewHolder> {
    ...
    public SiteAdapter(OnClickListener listener) {
        ...
        this.mItems = SiteConfig.get().getSites();
    }
    ...
    private void setEnable(boolean enable) {
        if (type == 1) for (Site site : SiteConfig.get().getSites()) site.setSearchable(enable).save();
        if (type == 2) for (Site site : SiteConfig.get().getSites()) site.setChangeable(enable).save();
        if (type == 3) for (Site site : SiteConfig.get().getSites()) site.setRecordable(enable).save();

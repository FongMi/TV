...
public class SiteDialog implements SiteAdapter.OnClickListener {
    ...
    private void setRecyclerView() {
        ...
        binding.recycler.post(() -> binding.recycler.scrollToPosition(SiteConfig.getHomeIndex()));
    }
    ...
    public boolean onSearchLongClick(Site item) {
        ...
        for (Site site : SiteConfig.get().getSites()) site.setSearchable(result).save();
    ...
    public boolean onChangeLongClick(Site item) {
        ...
        for (Site site : SiteConfig.get().getSites()) site.setChangeable(result).save();
        adapter.notifyItemRangeChanged(0, adapter.getItemCount());
        return true;
    }
}
